...
template <dimension_t DIMENSION> class MdTrieBench {
  public:
    MdTrieBench(md_trie<DIMENSION> *mdtrie) { mdtrie_ = mdtrie; };

    void insert(std::string data_addr, std::string outfile_name,
        ...
        while (std::getline(infile, line)) {
            ...
            if (n_points > total_points_count - points_to_insert)
                insertion_latency_vect_.push_back(latency +
                                                  SERVER_TO_SERVER_IN_NS);

            if (n_points == total_points_count)
                break;

            if (n_points % (total_points_count / 100) == 0)
                std::cout << n_points << " out of " << total_points_count
                          << std::endl;
        }

        std::cout << "Insertion Latency: " << (float)diff / n_points
                  << std::endl;
    ...
    void range_search(std::string query_addr, std::string outfile_name,
                      void (*get_query)(std::string, data_point<DIMENSION> *,
        ...
        for (int i = 0; i < QUERY_NUM; i++) {
            ...
            start = GetTimestamp();
            mdtrie_->range_search_trie(&start_range, &end_range,
                                       mdtrie_->root(), 0, found_points);
            diff = GetTimestamp() - start;
            outfile << "Query " << i
                    << " end to end latency (ms): " << diff / 1000
                    << ", found points count: "
                    << found_points.size() / DIMENSION << std::endl;
            found_points.clear();
        }
    }
    ...
    void get_storage(std::string outfile_name) {
        ...
        flush_string_to_file(std::to_string(size) + "," +
                                 std::to_string(total_points_count),
                             results_folder_addr + outfile_name);
    }
