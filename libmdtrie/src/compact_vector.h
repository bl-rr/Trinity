...
template <typename VectorImpl> class value_reference_vector {
  public:
...
// Iterators
template <typename VectorImpl> class vector_iterator {
  public:
    typedef typename VectorImpl::pos_type pos_type;
    ...
    bool operator==(const vector_iterator &it) const { return it.pos_ == pos_; }

    bool operator!=(const vector_iterator &it) const { return *this != it; }

    bool operator<(const vector_iterator &it) const { return pos_ < it.pos_; }
...
template <typename VectorImpl> class const_vector_iterator {
  public:
    typedef typename VectorImpl::pos_type pos_type;
    ...
    const_vector_iterator(const VectorImpl *array, pos_type pos) {
        array_ = array;
        pos_ = pos;
    }
...
template <typename T, uint8_t W> class CompactVector : public BitVector {
  public:
    ...
    explicit CompactVector(const CompactVector &vec)
        : BitVector(vec) // Explicitly initialize the base class
    {
        data_ = vec.data_;
        size_ = vec.size_;
    }
    ...
    void Append(T val) { this->AppendVal(val, W); }

    void Set(pos_type i, T value) { this->SetValPos(i * W, value, W); }

    T Get(pos_type i) const { return (T)this->GetValPos(i * W, W); }
    ...
    // Operators, iterators
    T operator[](const pos_type &i) const { return Get(i); }
...
class CompactPtrVector : CompactVector<uint64_t, 44> {
  public:
    ...
    CompactPtrVector(size_type num_elements)
        : CompactVector<uint64_t, 44>(num_elements) {
        num_elements_ = num_elements;
    }

    // Accessors, Mutators
    void *At(pos_type idx) {
        return reinterpret_cast<void *>(CompactVector<uint64_t, 44>::Get(idx)
                                        << 4ULL);
    }

    void Set(pos_type idx, void *val) {
        CompactVector<uint64_t, 44>::Set(idx, reinterpret_cast<uint64_t>(val) >>
                                                  4ULL);
    }
...
class CompactPrimaryVector : CompactVector<uint64_t, 46> {
  public:
    ...
    CompactPrimaryVector(size_type num_elements)
        : CompactVector<uint64_t, 46>(num_elements) {}

    // Accessors, Mutators
    uint64_t At(pos_type idx) {
        return reinterpret_cast<uint64_t>(CompactVector<uint64_t, 46>::Get(idx)
                                          << 20ULL);
    }

    void Set(pos_type idx, uint64_t val) {
        CompactVector<uint64_t, 46>::Set(idx, reinterpret_cast<uint64_t>(val) >>
                                                  20ULL);
    }
    ...
    get_delta_encoded_array_pointer(pos_type idx) {
        return (bitmap::EliasGammaDeltaEncodedArray<uint64_t> *)(ptr(idx)
                                                                 << 4ULL);
    }
    ...
    uint64_t size_overhead(pos_type idx) {
        ...
        if (flag(idx) == 0) {
            ...
            return get_delta_encoded_array_pointer(idx)
    ...
    void set_flag(pos_type idx, unsigned flag) { Set(idx, At(idx) & flag); }
};
