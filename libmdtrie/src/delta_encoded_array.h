    ...
    EliasGammaPrefixSum() {
        for (uint64_t i = 0; i < 65536; i++) {
            ...
            while (val && offset <= 16) {
                ...
                offset++;
                if (offset + N <= 16) {
                    sum += ((val >> offset) & ((uint16_t)low_bits_set[N])) +
                           (1 << N);
                    offset += N;
                    count++;
                } else {
...
class EliasGammaDeltaEncodedArray
  ...
  public:
    ...
    uint64_t size_overhead() {
        ...
        size +=
            this->samples_.GetSize() /*bitmap size*/ /* + sizeof(width_type)*/;
        ...
        while (delta_sum < val && current_delta_offset < delta_max &&
            ...
            if (block_cnt == 0) {
                ...
                while (delta_sum < val && current_delta_offset < delta_max &&
                    ...
                    current_delta_offset++;
                    last_decoded_value =
                        this->deltas_.GetValPos(current_delta_offset,
                                                delta_width) +
                        (1ULL << delta_width);
    ...
    bool BinarySearch(T search_val) {
        ...
        if (this->GetNElements() - sample_index * sampling_rate <
            sampling_rate) {
